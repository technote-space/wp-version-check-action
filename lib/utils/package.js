...
const updatePackageVersion = (logger, context) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const autoload = (0, misc_1.findAutoloadFile)(github_action_helper_1.Utils.getWorkspace());
    const results = yield constant_1.REPLACE_RULES.reduce((prev, rule) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        if (!(0, fs_1.existsSync)(config.file)) {
            return acc;
        }
        return acc.concat(yield (0, replace_in_file_1.replaceInFile)(Object.assign(Object.assign({}, config), { files: config.file })));
    }), Promise.resolve([]));
    logger.displayStdout((0, misc_1.getReplaceResultMessages)(results, logger));
    return results.filter(item => item.hasChanged).map(item => item.file);
});
exports.updatePackageVersion = updatePackageVersion;
const getUpdateBranch = (logger, context) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (tagName) {
        ...
        if (!(yield (0, command_1.getBranchesByTag)(tagName)).includes(branch)) {
...
const commit = (files, logger, octokit, context) => __awaiter(void 0, void 0, void 0, function* () {
    if (!files.length) {
    ...
    const branch = yield (0, exports.getUpdateBranch)(logger, context);
    if (false === branch) {
        return false;
    }
    ...
    yield helper.commit(github_action_helper_1.Utils.getWorkspace(), (0, misc_1.getCommitMessage)(), files);
